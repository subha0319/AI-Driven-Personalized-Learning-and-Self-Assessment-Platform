...
document.addEventListener("DOMContentLoaded", function () {
    ...
    addBookBtn.addEventListener("click", function () {
        const file = bookUploadInput.files[0]; // Get the selected file

        if (file && file.type === "application/pdf") {
            const bookName = file.name; // Get the name of the file (book)

            // Create a new element to display the selected book with a numbered order
            const bookElement = document.createElement("div");
            bookElement.classList.add("book-item");
            bookElement.innerHTML = `${bookCounter}. ${bookName} <span class="remove-book" data-id="${bookCounter}">  -remove-  </span>`;
            
            // Append the book item to the display area
            selectedBooksContainer.appendChild(bookElement);
            
            // Increment book counter for the next book
            bookCounter++;
            ...
            bookElement.querySelector(".remove-book").addEventListener("click", function () {
                selectedBooksContainer.removeChild(bookElement);
                updateBookNumbers(); // Update numbering after removal
            });
        } else {
    ...
    // Update the numbering of selected books
    function updateBookNumbers() {
        const books = selectedBooksContainer.querySelectorAll(".book-item");
        books.forEach((book, index) => {
            bookElement.classList.remove("book-item");
			book.innerHTML = `${index + 1} . ${book.innerHTML.split(".")[1]}`;
		});
        bookCounter = books.length + 1; // Adjust book counter after removal
    }
});
